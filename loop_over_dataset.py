...
results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', 'fpn-resnet','results_sequence_1_resnet')
# results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', 'fpn-resnet','results_sequence_2_resnet')
# results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', 'fpn-resnet','results_sequence_3_resnet')
datafile = WaymoDataFileReader(data_fullpath)
datafile_iter = iter(datafile)  # initialize dataset iterator

## Initialize object detection -- 사용할 모델을 선택, 최종 프로젝트에서 사용할 모델은 fpn_resnet
#                              -- Midterm project에서는 darknet(complex-YOLO)을 사용
...
configs_det.use_labels_as_objects = False # True = use groundtruth labels as objects
                                          #      >> True로 태그하면 성능 평가는 반드시 100%의 결과를 return
                                          # False = use model-based detection
                                          #      >> 성능평가와 같은 Task를 수행하기 위해 사용하는 Tag
# configs_det.save_results = False        # True : save results(binary) to file (based on data_filename)
                                          # 용도 : midterm project 에서 Stage 1,2,3,4 로 이루어져 있는데, stage 4 를 실행 시, 
                                          # 1-3 stage 결과를 불러와 stage 4의 Input으로 사용할 수 있다. (시간 절약을 위한 switch)
                                          
## Uncomment this setting to restrict the y-range in the final project
# configs_det.lim_y = [-25, 25] 


## Initialize tracking -- 매우 중요한 Part (추적)
...
## Selective execution and visualization -- 용도에 따라, 아래 명시된 options 를 적용하면 됨.
# 예를 들어 Point Cloud에서 BEV를 연산하려고 하면, ‘bev_from_pcl’을 복사해서 넣어주면 됨. ex) exec_detection = ['bev_from_pcl']

### Step 3.
# exec_data = ['pcl_from_rangeimage','load_image'] # options are 'pcl_from_rangeimage'
...
while True:
    try:
        ...
        #  아래는 실제 3D Object Detection을 수행하는 코드.
        ...
        # 위에서 exec_list 가 이미 생성되어 있음 - 구성 요소 : exec_detection, exec_tracking, exec_visualization
        # else :  >> exec_list가 정의되어 있지 않을 때. 미리 정의된 helper 함수를 이용해 Data를 Load함 ()
        #            즉 문자열을 입력해주지 않으면, 시스템은 기존의 저장되어 있는 File에서 Load 해오기를 원한다고 인식함
        ...
        ## Compute lidar birds-eye view (bev) # 실제 수행해야 할 코드 : Point Cloud -> BEV
        ...
        ## 3D object detection       
        #  tools.convert_labels_into_objects 라는 함수가 있지만, 실제 우리가 해야 할 Task는 저 함수를 사용하지 않고, 
        #  (처음 코드를 보면, config_det.use_labels_as_objects = True 로 정의되어 있음)
        #  >> else 단으로 넘어가서, det.detect_objects 라는 함수에서 코드 작업을 해야함.
        #  위의 경우와 마찬가지로, exec_list에서 tag를 정의해주지 않으면, 기존에 저장되어 있는 파일을 Load해옴.
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            if 'perform_tracking' in exec_list:
                ...
                # print(f'data_fullpath : {data_fullpath}\n results_fullpath : {results_fullpath}')
                # print(f'configs_det.arch : {configs_det.arch}')
