...
def show_pcl(pcl, flag = 0):
    ...
    vis1.create_window()
    vis1.register_key_callback(262, key_callback)
...
def show_range_image(frame, lidar_name):
    ...
    img_range_intensity = np.array(img_range_intensity.astype(np.uint8))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    # counts = lidar_pcl_cpy[:,0:2]
    
    # lidar_pcl_intensity = lidar_pcl_cpy[indices]
    # print(f'3 : {lidar_pcl_intensity.shape}')
    # lidar_pcl_top = lidar_pcl_intensity
    ...
    lidar_pcl_hei = lidar_pcl_hei[indices]
    ...
    height_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 2] / (np.amax(lidar_pcl_top[:, 2])-np.amin(lidar_pcl_top[:, 2]))
...
#     vis.register_key_callback(262, my_key_callback)
...
#     # step 1 : extract lidar data and range image for the roof-mounted lidar
#     lidar=[obj for obj in frame.lasers if obj.name==lidar.name][0]
#     ri=[]
...
#     # step 4 : map the range channel onto an 8-bit scale and make sure that the full range of values is appropriately considered
#     ri_range=ri_range*255/(np.amax(ri_range)-np.amin(ri_range))
#     img_range=ri.range.astype(np.uint8)
#     # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
#     percentile_1,percentile_99=np.percentile(ri_intensity,1),np.percentile(ri_intensity,99)
#     ri_intensity=255*np.clip(ri_intensity,percentile_1,percentile_99)/percentile_99
#     # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
...
#     ri_center=int(img_range.shape[1]/2)
#     img_range_intensity=img_range_intensity[:,ri_center-deg90:ri_center+deg90]
#     cv2.imshow("img_range",img_range_intensity)
#     cv2.waitKey(0)
...
#     bev_maps = torch.from_numpy(bev_maps)  # create tensor from birds-eye view
#     input_bev_maps = bev_maps.to(configs.device, non_blocking=True).float()
#     return input_bev_maps# print(pcl.shape)
